exports = module.exports = function(Discourse, actionTypeEnum) {
  ...
  Discourse.prototype.getTopicAndReplies = function(topic_id, filter, callback) {
    if (typeof filter === 'function') {
      callback = filter;
      filter = {};
    }
    var path = 't/' + topic_id;
    //go to specific post_number
    if(filter.post_number){
      path = path +'/' + filter.post_number
      delete filter.post_number;
    }
    this.get(path + '.json', filter,
      function(error, body, httpCode) {
        callback(error, body, httpCode);
      }
    );
  ...
  Discourse.prototype.likePost = function(post_id, callback) {
    this.post('post_actions', { 'id': post_id, 'post_action_type_id': 2, 'flag_topic': false }, function(error, body, httpCode) {
      callback(error, body, httpCode);
    });
  };

  Discourse.prototype.unlikePost = function(post_id, callback) {
    this.delete('post_actions/'+post_id, {'post_action_type_id': 2 }, function(error, body, httpCode) {
      callback(error, body, httpCode);
    });
  };
